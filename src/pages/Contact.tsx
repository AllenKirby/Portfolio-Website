...
const Contact = () => {
    ...
    const [contactTitleRef, contactTitleInView] = useInView<HTMLHeadingElement>()
    const [descriptionRef, descriptionInView] = useInView<HTMLHeadingElement>()
    const [iconsRef, iconsInView] = useInView<HTMLDivElement>()
    const [formRef, formInView] = useInView<HTMLFormElement>()
    ...
    <section id="contact" className="w-full h-screen flex flex-col lg:flex-row items-center justify-center">
        <section className="w-full lg:w-1/2 h-fit lg:h-full p-10 flex items-center justify-center">
            <main className="flex flex-col items-center lg:items-start justify-center">
                <motion.h1 
                    ...
                    animate={contactTitleInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.75 }}
                    transition={{ duration: 0.5, ease: 'easeInOut' }}
                    className="text-3xl font-bold mb-2 bg-gradient-to-r from-secondary to-white bg-clip-text text-transparent"
                    >
                        Contact Me
                </motion.h1>
                <motion.p 
                ...
                <motion.div 
                    ref={iconsRef}
                    initial={{ opacity: 0, scale: 0.75 }}
                    animate={iconsInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.75 }}
                    transition={{ duration: 0.5, ease: 'easeInOut' }}
        ...
        <div className="w-full lg:w-1/2 h-fit lg:h-full flex items-start lg:items-center justify-center ">
            <motion.form  
                ref={formRef}
                initial={{ opacity: 0, scale: 0.75 }}
                animate={formInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.75 }}
                transition={{ duration: 0.5, ease: 'easeInOut' }}
                onSubmit={handleSubmit} className="w-5/6 h-fit flex flex-col gap-4 p-10 rounded-md border border-white text-white"
                >
