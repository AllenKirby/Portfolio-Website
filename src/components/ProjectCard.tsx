...
const ProjectCard: React.FC<ProjectCardState> = React.memo((props) => {
    ...
    const [projectRef, projectInView] = useInView<HTMLElement>()
  ...
  return (
    <motion.section 
        ...
        animate={projectInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.75 }}
        transition={{ duration: 0.5, ease: 'easeInOut' }}
        className='w-full h-fit flex flex-col lg:flex-row gap-10 my-10'
        >
            {index % 2 === 0 && (
